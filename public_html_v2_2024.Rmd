---
title: "Examining the Decline of Save Percentage in the NHL (2024)"
...
#Load PBP using Hockey R, filter and write to CSV
...
# write_csv(pbp, 'pbp.csv')


# pbp24 <- load_pbp(season = 2012:2024) %>%
#    mutate(empty_net = ifelse(is.na(empty_net) | empty_net == FALSE, FALSE, TRUE)) %>%
#    filter(season_type == 'R',
#           period_type != "SHOOTOUT",
#           empty_net == FALSE)
# pbp24$season <- substr(pbp24$season, 5, 8)
...
#          -detailed_state, -venue_id, -contains("venue"), -contains("division"), -contains("conference"), -contains("abbreviation"),
#          -contains("ids"), -event_idx, -ordinal_num, -home_id, -away_id, -event_goalie_id, -event_goalie_link, -event_goalie_type)
# write_csv(pbp24, 'pbp24.csv')

```


```{r, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
goalies24 <- read_csv("data\\goalies_all_seasons_count_24.csv") %>% 
  rename(season = Season, team = Tm)

goalies_team24 <- goalies24 %>% 
  filter(team != "TOT")

goalies_indv24 <- goalies24 %>%
  group_by(Player, season, Rk) %>% 
  filter(row_number()==1)

sample_shot <- read_csv("data\\shot-sample-updated.csv") %>% mutate(season = substr(season, 5, 8))
...
```{r, echo=FALSE}
svpct_by_year24 <- goalies_indv24 %>% 
  group_by(season) %>%
...
svpct_by_year24 %>% 
  ...
  geom_point(size = 3) +
  geom_line() +
  scale_x_continuous("Season", labels = svpct_by_year24$season, breaks = svpct_by_year24$season) +
...
svpct_by_year24 %>%
  ...
  column_spec(5, color = "white",
              background = spec_color(svpct_by_year24$SVpct, end = 0.7),
              popover = paste("am:", svpct_by_year24$SVpct)) 
```
...
```{r, echo = FALSE, message=FALSE, warning=FALSE}
#starters ####
starters24 <- goalies_indv24 %>% 
  group_by(season) %>% 
  arrange(-GS, -`SV%`) %>% 
  slice(1:30) %>% 
  mutate(rank = "Starter")

games_per_season24 <- pbp24 %>% 
...
starters24 <- games_per_season24 %>%
  left_join(teams_per_season24, by = "season") %>% 
  mutate(games_per = (games*2)/teams,
         season = as.numeric(season)) %>% 
  right_join(starters24, by = "season")

starters24$games_per[which(starters24$season == 2023)] <-  82

pctstarts24 <- starters24 %>% 
...
pctstarts24 %>% 
  ...
  scale_x_continuous("Season", labels = pctstarts24$season, breaks = pctstarts24$season) +
...
pctstarts24 %>%
  ...
  column_spec(3, color = "white",
              background = spec_color(pctstarts24$pct_started, end = 0.7),
              popover = paste("am:", pctstarts24$pct_started))
```
...
```{r, echo = FALSE, message=FALSE, warning = FALSE}
backups24 <- goalies_indv24 %>% 
  group_by(season) %>% 
  arrange(-GS, -`SV%`) %>% 
  slice(31:1000) %>% 
  mutate(rank = "Backup")

backups24 <- games_per_season24 %>%
  left_join(teams_per_season24, by = "season") %>% 
  mutate(games_per = (games*2)/teams,
         season = as.numeric(season)) %>% 
  right_join(backups24, by = "season")

backups24$games_per[which(backups24$season == 2023)] <-  82

startback24 <- starters24 %>% 
  rbind(., backups24) %>%
...
startback24 %>% 
  ...
  scale_x_continuous("Season", labels = startback24$season, breaks = startback24$season) +
  ylab("Save Percentage") +
  labs(color = "Rank") +
...
avgs <- read_csv("data\\league_avgs.csv")
...
xG_by_year24 <- function(year) {
  ...
  pbp_year <- pbp24 %>% filter(season == year)
...
years24 <- c("2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019", "2020", "2021", "2022", "2023", "2024")

coefs_ev24 <- data.frame()
for(season in years24) {
  year_model <- xG_by_year24(season)
  ...
  colnames(coef_year_df) = c('X_Int', 'Shot_Distance', 'Shot_Angle', 
                             'Rush', 'Rebound', 'Cross_Ice', 'Forecheck', 'Rebound_Angle_Change',
                             'X_Int_SD', 'Shot_Distance_SD', 'Shot_Angle_SD',
                             'Rush_SD', 'Rebound_SD', 'Cross_Ice_SD', 'Forecheck_SD', 'Rebound_Angle_Change_SD')
  
  coefs_ev24 <- rbind(coefs_ev24, coef_year_df)
  print(season)
  
}

coefs_ev24$season <- years24
```


```{r, echo=FALSE}
#Power Play Model ####
xG_by_year_pp24 <- function(year) {
...
coefs_pp24 <- data.frame()
for(season in years24) {
  year_model <- xG_by_year_pp24(season)
  year_model_coefs <- summary(year_model)$coefficients[,1:2]
  coef_year_df <- year_model_coefs %>% 
    as.list() %>% 
    data.frame(.)
  ...
  coefs_pp24 <- rbind(coefs_pp24, coef_year_df)
  
  print(season)
  
}

coefs_pp24$season <- years24
```
...
ev_dist24$`Shot Distance` <- factor(ev_dist24$`Shot Distance`, levels=seq(0, 60, 10))
...
pp_dist24$`Shot Distance` <- factor(pp_dist24$`Shot Distance`, levels=seq(0, 60, 10))

my_colors = carto_pal(7, "Bold")

pp_dist_gg24 <- pp_dist24 %>% 
  ggplot(aes(x = as.numeric(season), y = value, color = `Shot Distance`)) + 
  geom_point() + 
  geom_line() + 
  scale_x_continuous("Season", labels = as.numeric(pp_dist24$season), breaks = as.numeric(pp_dist24$season)) +
  labs(y = "Proportion of Total Shots", title = "Shot Distance Distribution by Year on Power Plays")  + 
  scale_color_manual(values = my_colors) +
  theme_bw()

ev_dist_gg24 <- ev_dist24 %>% 
  ggplot(aes(x = as.numeric(season), y = value, color = `Shot Distance`)) + 
  geom_point() + 
  geom_line() + 
  scale_x_continuous("Season", labels = as.numeric(ev_dist24$season), breaks = as.numeric(ev_dist24$season)) +
  labs(y = "Proportion of Total Shots", title = "Shot Distance Distribution by Year with Even Strength") +
  ylim(ggplot_build(pp_dist_gg24)$layout$panel_scales_y[[1]]$range$range) +
  scale_color_manual(values = my_colors) +
  theme_bw()
...
ev_angle24 %>% 
  ...
  geom_point() + 
  geom_line() + 
  scale_x_continuous("Season", labels = as.numeric(ev_angle24$season), breaks = as.numeric(ev_angle24$season)) +
...
ev_rebound24 <- ev24 %>% 
  ...
  labs(title = 'Even Strength Rebound Frequency by Year', 
       ...
       y = 'Percentage of All Shots in Season',
       fill = 'Shot Result')+ 
  ylim(ggplot_build(pp_rebound24)$layout$panel_scales_y[[1]]$range$range) +
  scale_fill_manual(values = my_colors) +
  theme_bw()
...
pp24 %>% 
  ...
  labs(title = 'Power Play Rush Frequency by Year', 
       ...
       y = 'Percentage of All Shots in Season',
       fill = 'Shot Result') + 
  ylim(ggplot_build(ev_rush24)$layout$panel_scales_y[[1]]$range$range) +
  scale_fill_manual(values = my_colors) +
  theme_bw()
```
...
#   summarize(cross_ice_event = mean(cross_ice_event)) %>% 
  ...
  labs(title = 'Power Play Cross Ice Event Frequency by Year', 
       ...
       y = 'Percentage of All Shots in Season',
       fill = 'Shot Result') +
  ylim(ggplot_build(ev_cross24)$layout$panel_scales_y[[1]]$range$range) +
  scale_fill_manual(values = my_colors) +
  theme_bw()
...
ev_reb_angle24$`Angle Change` <- factor(ev_reb_angle24$`Angle Change`, levels=seq(0, 160, 20))

ev_reb_angle24 %>% 
  ggplot(aes(x = as.numeric(season), y = value, color = `Angle Change`)) + 
  geom_point() + 
  geom_line() + 
  scale_x_continuous("Season", labels = as.numeric(ev_reb_angle24$season), breaks = as.numeric(ev_reb_angle24$season)) +
...
pp_reb_angle24$`Angle Change` <- factor(pp_reb_angle24$`Angle Change`, levels=seq(0, 160, 20))

pp_reb_angle24 %>% 
  ggplot(aes(x = as.numeric(season), y = value, color = `Angle Change`)) + 
  geom_point() + 
  geom_line() + 
  scale_x_continuous("Season", labels = as.numeric(pp_reb_angle24$season), breaks = as.numeric(pp_reb_angle24$season)) +
...
pub_xG_by_year <- function(year) {
  ...
  logit <- pbp_shots %>% select(event_type, shot_distance, shot_angle, rush,
                                rebound, cross_ice_event, forecheck, angle_change, goal)
  # logit$xg <- pbp_shots$xg
  logit2 <- logit %>% na.omit()
...
xG_by_year <- function(year) {
  ...
  init_logit <- glm(is_goal ~ shot_distance + abs(shot_angle) + rush + is_rebound +
                      cross_ice + off_the_forecheck + is_rebound:reb_angle_change,
